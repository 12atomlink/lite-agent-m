...
export namespace SessionPrompt {
  ...
  export const loop = fn(LoopInput, async (input) => {
    ...
    log.info("loop.start", { sessionID, resume_existing, time: loopStartTime })
    ...
    while (true) {
      ...
      log.info("loop.step.start", { step, sessionID, elapsed: stepStartTime - loopStartTime })
      ...
      step++
      if (step === 1) {
        log.info("loop.ensureTitle.start", { sessionID, elapsed: Date.now() - loopStartTime })
        ensureTitle({
          session,
      ...
      log.info("loop.resolveTools.start", { sessionID, elapsed: Date.now() - loopStartTime })
      ...
      log.info("loop.resolveTools.done", { sessionID, elapsed: Date.now() - loopStartTime, toolCount: Object.keys(tools).length })
      ...
      log.info("loop.skills.start", { sessionID, elapsed: Date.now() - loopStartTime })
      const skills = await SystemPrompt.skills(agent)
      log.info("loop.skills.done", { sessionID, elapsed: Date.now() - loopStartTime })
      const system = [
      ...
      log.info("loop.llm.start", { sessionID, elapsed: Date.now() - loopStartTime })
      ...
      log.info("loop.llm.done", { sessionID, elapsed: Date.now() - loopStartTime, result })
